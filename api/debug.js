  ...
  try {
    ...
    if (!q.length) {
      verdict = out.reads[QUEUE_TAB] && out.reads[QUEUE_TAB].rowCount > 1
        ? 'ROWS EXIST IN THE SHEET BUT NONE SURVIVED PARSING — every row is missing lead_id or category. Check that the Audit_Queue header row matches the v4 column names exactly.'
        : 'NO ROWS AT SOURCE — the Metabase card and the ' + QUEUE_TAB + ' tab both came back empty. Run audit_queue_daily_v4.sql and paste the export in, or check METABASE_CARD.';
    } else if (!H.length) {
      ...
      verdict = 'JOIN FAILURE — ' + q.length + ' queue rows and ' + H.length + ' mapping rows, but NOT ONE lrm_email matches. Compare sampleQueueLrmEmails with sampleMapLrmEmails: usually the query emits a different address form (or blank) than the mapping sheet.';
    } else if (blankLrm) {
      verdict = 'PARTIAL — ' + matched.length + ' LRMs join, but ' + blankLrm + ' queue rows carry a blank lrm_email and can never be assigned to a TL. Those leads are invisible to everyone.';
    } else {
