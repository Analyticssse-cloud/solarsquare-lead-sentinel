-- audit_queue_daily_v7_8.sql — v6 plus the stage blocklist and a 6-month lead window
...
-- ===========================================================================
-- v7.7 (3 Aug 2026) — ONLY 'Lead - Not Interested' IS AN NI CLOSE
--   'Meeting - Not Interested' and 'Lead - Not Interested' are two different stages and
--   only the second one belongs to this desk. v7.6's '%not interested%' ILIKE matched
--   both, so a post-meeting NI was being dated and audited as a lead-stage close.
--   Two changes:
--     · is_ni and the terminal_at ni_at date now match 'lead%not interested%' only.
--     · 'Meeting - Not Interested' is blocklisted, so removing it from is_ni cannot
--       drop it through into a follow-up or meeting branch instead.
--   NQ IS DELIBERATELY NOT SPLIT THE SAME WAY. Confirmed 3 Aug 2026: both
--   'Lead - Not Qualified' AND 'Meeting - Not Qualified' are to be captured, so is_nq
--   and nq_at keep their '%not qualified%' ILIKE. Do not "align" this with is_ni.
-- ===========================================================================
-- v7.8 (3 Aug 2026) — THE FOLLOW-UP DATE WAS READ FROM THE WRONG COLUMN
--   Reported on LMP117365: stage 'Future Followup Required', the CRM shows the follow-up
--   as 05-11-2026 09:30 AM, and the queue emitted 2026-07-05T09:00:00Z. Two faults, and
--   they compounded:
--
--   1. lrm_queue_entry_next_action_at WAS READ FIRST, AND IT IS NOT THE FOLLOW-UP DATE.
--      On this lead it holds 2026-07-05T09:00:00Z — 14:30 IST on the day the lead was last
--      modified (lead_last_modified_on 2026-07-05T07:23:51Z), not 09:30 IST on 5 Nov. The
--      lrm_queue_entry_* family is current-queue-state and is overwritten in place; it
--      records when the queue last handled the lead, not what the LRM committed to.
--      Demoted below the reschedule fields.
--
--   2. THE reschedule_date BRANCH WAS DEAD CODE AND HAD NEVER FIRED.
--      v7.1 assumed reschedule_date was DD-MM-YYYY text and guarded it with
--      '^\d{1,2}-\d{1,2}-\d{4}$'. lead.reschedule_date is NATIVE timestamptz, so
--      ::text renders '2026-11-05 00:00:00+00' and the guard never matched — the branch
--      returned NULL for every lead in the table. The correct date was sitting right there
--      and the COALESCE could never reach it. Used natively now, no regex.
--
--   The follow-up instant is now composed as reschedule_date (IST calendar date) +
--   reschedule_time (the CRM's 'HH:MI AM' text, 09:00 IST if absent or malformed), which
--   reproduces the value the LRM sees in Lighthouse. Order is now:
--      reschedule_date (+time)  →  lrm_queue_entry_next_action_at  →  follow_up_datetime
--
--   CALL LATER, checked at the same time: it was never a stage-driven arm — it reached the
--   queue only through the generic fu_ist rule, so it inherited exactly the same wrong
--   date. It now has an explicit arm alongside Future Followup Required, and both read the
--   corrected fu_ist. reschedule_date / reschedule_time are emitted in the output so an
--   auditor can check the queue's date against the CRM without opening a second tab.
-- ===========================================================================
WITH cfg AS (
  SELECT
...
excluded_stages AS (
  SELECT LOWER(x) AS stage_lc FROM unnest(ARRAY[
    ...
    'Speed Order',
    -- ★ v7.7: a post-meeting NI is a different stage from the lead-stage close and is not
    --   this desk's. Blocked so it cannot fall through to another branch now that is_ni
    --   no longer claims it.
...
terminal_at AS (
  SELECT a.lead_id,
         -- ★ v7.7: 'lead%' prefix — 'Meeting - Not Interested' must not date an NI close.
         MAX(a."createdAt") FILTER (WHERE COALESCE(a.stage,'') ILIKE 'lead%not interested%') AS ni_at,
...
enriched AS (
  SELECT
    ...
    -- THE LRM'S REAL NEXT ACTION. See the v7.8 header: v7.1 read this from
    -- lrm_queue_entry_next_action_at with a dead reschedule_date branch underneath it, and
    -- so reported the queue's own timestamp as the LRM's committed follow-up date.
    COALESCE(
      -- ★ v7.8: the committed follow-up, exactly as Lighthouse shows it. Native timestamptz
      --   (no regex — the v7.1 DD-MM-YYYY guard was wrong and never matched), taken as an
      --   IST calendar date and re-timed from reschedule_time. 09:00 IST when the time is
      --   missing or unparseable, so the date still lands on the right day.
      CASE WHEN b.reschedule_date IS NOT NULL
           THEN (((b.reschedule_date AT TIME ZONE 'Asia/Kolkata')::date
                   + COALESCE(
                       CASE WHEN btrim(COALESCE(b.reschedule_time::text, '')) ~* '^\d{1,2}:\d{2}\s*[AP]M$'
                            THEN TO_TIMESTAMP(btrim(b.reschedule_time::text), 'HH12:MI AM')::time END,
                       TIME '09:00'))
                 AT TIME ZONE 'Asia/Kolkata') END,
      -- ★ v7.8: demoted. Queue-entry state, overwritten in place — it dates when the queue
      --   last touched the lead, not what the LRM promised the customer. Fallback only.
      b.lrm_queue_entry_next_action_at,
      b.follow_up_datetime
    )                                                        AS follow_up_at,
    b.follow_up_datetime                                     AS follow_up_datetime_raw,
    b.reschedule_date                                        AS reschedule_date_raw,
    b.reschedule_time::text                                  AS reschedule_time_raw,
...
acted AS (
  SELECT a.*,
    ...
    -- ★ v7.7: 'Lead - Not Interested' only. 'Meeting - Not Interested' is a separate stage
    --   and is blocklisted above rather than audited here.
    (COALESCE(current_stage, '') ILIKE 'lead%not interested%')                AS is_ni,
...
categorised AS (
  SELECT c.*,
    CASE
      ...
      -- ★ v7.8: Call Later, same shape. Previously it had no arm of its own and was only
      --   ever picked up by the generic fu_ist rule below, which meant it carried the same
      --   wrong follow-up date. Named explicitly so the two committed-callback stages are
      --   read the same way.
      WHEN stage_lc LIKE '%call later%'
           AND fu_ist IS NOT NULL AND fu_ist <= today               THEN 'followup_missed'
...
SELECT
  ...
  follow_up_at, follow_up_datetime_raw,
  -- ★ v7.8: the raw CRM fields the follow-up date is built from, so a disagreement with
  --   Lighthouse is visible in the row instead of needing a second lookup.
  reschedule_date_raw, reschedule_time_raw,
  qualified_at, status_changed_at, attempt_today,
